	...
	while True:
		...
		for ((top, right, bottom, left), name) in zip(boxes, names):
			...
			if name == "Sarishma" :
				duration = 1  # seconds
				freq = 440  # Hz
				os.system('spd-say "Criminal Detected"'.format(duration, freq))
			...
			# alert 
				...
				print("Unkown face detected at %s" % str(time.ctime()))
			#	save the image 
			#	cv2.imwrite('/tmp/unkown-' + str(time.time()) + '.jpg', frame)
		...
		# the screen
		if args["display"] > 0:
			cv2.imshow("Recognizing: Press Escape key to exit", frame)
